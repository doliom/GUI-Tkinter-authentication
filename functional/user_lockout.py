...
class Lockout:
    def __init__(self, root, row1):
        ...
        Label(text="", bg="LightYellow").pack()
        unblock = Radiobutton(text="unblock", bg="LightYellow", fg="black", variable=self.is_blocked, value=0)
        block = Radiobutton(text="block", bg="LightYellow", fg="black", variable=self.is_blocked, value=1)
        ...
        with open(path, "r+") as f:
            ...
            for row in new_f:
                if username not in row:
                    ...
                    user_information[3] = self.is_blocked.get()
            f.truncate()
