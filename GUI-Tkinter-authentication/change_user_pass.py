...
class ChangeUserPassword:
    ...
    def back_to_prev(self):
        ...
        a_screen = User(screen, self.row)
        screen.mainloop()

    def update_pass(self):
        if self.row[1] == self.old_password.get():
            if self.row[4] == False or (self.row[4] == True and self.is_allowed(self.new_password.get()) == True):
                username = self.row[0]
                ...
                user_information = [username, self.new_password.get(), len(self.new_password.get()), self.row[3], self.row[4]]
                ...
                messagebox.showerror(title="Wrong password", message="Admin have set password restrictions")
        else:
